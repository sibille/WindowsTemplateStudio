    ...
    public abstract class GenController
    {
        ...
        internal async Task<Dictionary<string, TemplateCreationResult>> GenerateItemsAsync(IEnumerable<GenInfo> genItems, bool isTempGeneration)
        {
            ...
            foreach (var genInfo in genItems)
            {
                ...
                await ExecutePostActions(genInfo, result);
            }

            chrono.Stop();
            CalculateGenerationTime(chrono.Elapsed.TotalSeconds);

            await ExecuteGlobalPostActions();
        ...
        internal async Task ExecutePostActions(GenInfo genInfo, TemplateCreationResult generationResult)
        {
        ...
        internal async Task ExecuteGlobalPostActions()
        {
